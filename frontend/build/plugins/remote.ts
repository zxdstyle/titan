...
export default function remoteModulePlugin() {
	...
	const downloadCache: Record<string, boolean> = {};
	const resolveIdCache = {};

	function getLocalFileByUrl(remoteUrl: string) {
	...
	function parseUrl(id) {
		const [url] = id.match(/https?.*?$/gim) || [];
		return url;
	}

	// todo FIXME 动态加载时 某些url会重复下载
	function downloadFile(remoteUrl: string) {
	...
	function isRemoteModuleId(id: string) {
...
export function loadRemoteComponent(url) {
  ...
  const {hostname, pathname, port} = uri
  const host = hostname + '_' + port
